  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-md animate-fadeIn font-sans">
        <div className="bg-white dark:bg-slate-900 w-[calc(100%-16px)] sm:w-full max-w-6xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[92vh]">
            ...
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="min-w-0">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-mono text-[11px] sm:text-xs font-extrabold text-blue-600 bg-blue-50 dark:bg-blue-950 px-2 py-0.5 rounded-lg border border-blue-200 shrink-0">
                    {student.studentId}
                  </span>
                </div>
          ...
          {/* Main Scroll Content */}
          <div className="p-4 sm:p-6 space-y-5 overflow-y-auto flex-1 bg-slate-50/50 dark:bg-slate-950/50">
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-3 shadow-xs">
              <div className="flex items-center justify-between text-xs font-bold">
                ...
                <span className="text-blue-600 dark:text-blue-400 font-extrabold">{uploadedCount} / {totalRequired} ({completionPercentage}%)</span>
              </div>
            ...
            {/* Certificates List: Mobile Stacked Cards (< md) + Desktop Table (md+) */}
            <div className="border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-xs">
                    ...
                    <div key={idx} className="p-4 space-y-2.5">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center space-x-2">
                          ...
                          <h4 className="text-xs font-bold text-slate-900 dark:text-white">{item.typeName}</h4>
                        </div>
                        {item.isUploaded ? (
                          ...
                          <span className="px-2 py-0.5 bg-rose-50 text-rose-700 rounded-full text-[9px] font-extrabold uppercase shrink-0">
                            MISSING
                          </span>
                        )}
                      </div>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200/80 dark:border-slate-800 text-[11px] font-extrabold uppercase text-slate-500 whitespace-nowrap">
                            ...
                            <td className="p-3.5 pr-6 text-right space-x-1.5 whitespace-nowrap">
                              {item.isUploaded ? (
                                <>
                                  <button
                                    onClick={() => setSelectedDocForPreview(doc)}
                                    className="p-1.5 text-slate-400 hover:text-blue-600 rounded-lg hover:bg-slate-100 transition cursor-pointer min-w-[32px] min-h-[32px] inline-flex items-center justify-center"
                                  ...
                                  {doc.previewUrl && (
                                    <a
                                      ...
                                      download
                                      className="p-1.5 text-slate-400 hover:text-blue-600 rounded-lg hover:bg-slate-100 transition inline-flex items-center justify-center min-w-[32px] min-h-[32px]"
                                  ...
                                  {isAdmin && doc.status !== 'VERIFIED' && (
                                    <button
                                      onClick={() => handleVerify(doc.id)}
                                      className="p-1.5 text-slate-400 hover:text-emerald-600 rounded-lg hover:bg-emerald-50 transition cursor-pointer min-w-[32px] min-h-[32px] inline-flex items-center justify-center"
                                  ...
                                  {isAdmin && doc.status !== 'REJECTED' && (
                                    <button
                                      ...
                                      className="p-1.5 text-slate-400 hover:text-rose-600 rounded-lg hover:bg-rose-50 transition cursor-pointer min-w-[32px] min-h-[32px] inline-flex items-center justify-center"
                                ...
                                <button
                                  ...
                                  className="py-1 px-3 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-xl text-xs font-bold transition inline-flex items-center space-x-1 cursor-pointer min-h-[32px]"
                                >
                                  <Plus className="w-3.5 h-3.5" />
                                  <span>Upload</span>
                                </button>
      ...
      {showUploadModal && (
        <UploadCertificateModal
          ...
          prefilledDocTypeId={prefilledDocTypeId}
      ...
      {docToDelete && (
        <DeleteConfirmationModal
          ...
          subtitle="Certificate File & Metadata Purge"
          entityDetails={[
            { label: 'Document Type', value: docToDelete.documentTypeName || 'Certificate' },
            { label: 'Student ID', value: student.studentId },
            { label: 'Student Name', value: student.fullName },
            { label: 'File Name', value: docToDelete.originalFileName || 'PDF File' },
          ]}
          warningList={[
            'Permanently deletes the PDF file from storage bucket',
            'Certificate status will revert back to MISSING for this student',
          ]}
