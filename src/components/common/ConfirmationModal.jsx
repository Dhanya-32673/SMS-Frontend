...
export const ConfirmationModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-slate-950/70 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col font-sans max-h-[92vh]">
        {/* Header */}
        ...
        {/* Content */}
        <div className="p-4 sm:p-6 space-y-4 text-xs text-slate-600 dark:text-slate-300">
          ...
          {/* Footer Actions */}
          <div className="pt-2 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5 sm:gap-3 border-t border-slate-100 dark:border-slate-800">
            <button
              autoFocus
              type="button"
              onClick={onClose}
              disabled={loading}
              className="py-2.5 px-4 text-xs font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition cursor-pointer disabled:opacity-50 min-h-[44px] w-full sm:w-auto text-center"
            >
              {cancelText}
            </button>
            ...
            <button
              ...
              onClick={onConfirm}
              disabled={loading}
              className={`py-2.5 px-5 text-xs font-bold rounded-xl shadow-md flex items-center justify-center space-x-2 transition cursor-pointer disabled:opacity-50 min-h-[44px] w-full sm:w-auto ${btnBg}`}
            >
              {loading ? (
                <>
