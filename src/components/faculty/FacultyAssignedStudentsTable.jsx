...
export const FacultyAssignedStudentsTable = ({ studentList }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs overflow-hidden font-sans">
      ...
      <div className="block md:hidden divide-y divide-slate-100 dark:divide-slate-800">
        {studentList && studentList.length > 0 ? (
          ...
          <div className="py-8 text-center text-xs text-slate-400">No students assigned to your sections yet.</div>
        )}
      </div>
              ...
              studentList.map((st) => (
                <tr key={st.id || st.studentId} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition">
                  ...
                  <td className="p-3.5 pr-6 text-right">
                    <button
                      onClick={() => navigate(`/faculty/students/${st.studentId || st.id}`)}
                      className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/50 rounded-lg transition cursor-pointer min-w-[36px] min-h-[36px] inline-flex items-center justify-center"
